# Implementer Brief — Question bank, Tier 1 (Rookie) → `src/questions-t1.js`
...
2. **`src/curriculum-t1.js`** — the four chapters you are writing questions for.
...
5. The KB pages that `src/curriculum-t1.js` cites in its chapters, under
   `/Users/jschasse/knowledge-base/youth-baseball-canada/wiki/`.

## Deliverable

**One file: `src/questions-t1.js`**, calling `HRL_QUESTIONS.register([...])`
with questions for chapters **ch01, ch02, ch03, ch04**, all `tier: 'rookie'`.
...
- Ids: `q0101`, `q0102`, … per chapter, contiguous from 01.
- The first 6–8 ids of each chapter **must be exactly that chapter's `quizIds`**
  as written in `src/curriculum-t1.js`. Read them; do not assume.
- Mean difficulty for this tier ≈ **1.6**, with genuine spread inside it.
- At least one `hotspot` question and one `order` question in this file.
- Topics here skew `field`, `positions`, `rules`, and `safety` — but include a few `hitting`/`baserunning` basics.

## Acceptance

- File exists, is an ES5-safe IIFE, loads in Node, registers ≥48 questions.
- Every id is unique and follows the `q<CC><NN>` pattern.
- Every `chapter` value is one of ch01, ch02, ch03, ch04; every `tier` is `'rookie'`.
