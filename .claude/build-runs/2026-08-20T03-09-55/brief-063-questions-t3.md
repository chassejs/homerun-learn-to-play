# Implementer Brief — Question bank, Tier 3 (Diamond) → `src/questions-t3.js`
...
2. **`src/curriculum-t3.js`** — the four chapters you are writing questions for.
...
5. The KB pages that `src/curriculum-t3.js` cites in its chapters, under
   `/Users/jschasse/knowledge-base/youth-baseball-canada/wiki/`.

## Deliverable

**One file: `src/questions-t3.js`**, calling `HRL_QUESTIONS.register([...])`
with questions for chapters **ch09, ch10, ch11, ch12**, all `tier: 'diamond'`.
...
- Ids: `q0901`, `q0902`, … per chapter, contiguous from 01.
- The first 6–8 ids of each chapter **must be exactly that chapter's `quizIds`**
  as written in `src/curriculum-t3.js`. Read them; do not assume.
- Mean difficulty for this tier ≈ **4.8**, with genuine spread inside it.
- At least one `hotspot` question and one `order` question in this file.
- Topics skew `hitting`, `pitching`, `fielding`, and `safety` (arm care).
- Include **≥3 `scenario` questions**.
- At least three questions must test age-appropriate progression — specifically
  what should NOT be taught yet at a given age band, sourced from the KB skill roadmaps.

## Acceptance

- File exists, is an ES5-safe IIFE, loads in Node, registers ≥48 questions.
- Every id is unique and follows the `q<CC><NN>` pattern.
- Every `chapter` value is one of ch09, ch10, ch11, ch12; every `tier` is `'diamond'`.
