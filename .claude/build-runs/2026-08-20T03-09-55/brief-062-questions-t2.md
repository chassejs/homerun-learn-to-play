# Implementer Brief — Question bank, Tier 2 (Sandlot) → `src/questions-t2.js`
...
2. **`src/curriculum-t2.js`** — the four chapters you are writing questions for.
...
5. The KB pages that `src/curriculum-t2.js` cites in its chapters, under
   `/Users/jschasse/knowledge-base/youth-baseball-canada/wiki/`.

## Deliverable

**One file: `src/questions-t2.js`**, calling `HRL_QUESTIONS.register([...])`
with questions for chapters **ch05, ch06, ch07, ch08**, all `tier: 'sandlot'`.
...
- Ids: `q0501`, `q0502`, … per chapter, contiguous from 01.
- The first 6–8 ids of each chapter **must be exactly that chapter's `quizIds`**
  as written in `src/curriculum-t2.js`. Read them; do not assume.
- Mean difficulty for this tier ≈ **3.2**, with genuine spread inside it.
- At least one `hotspot` question and one `order` question in this file.
- Topics skew `rules` and `baserunning`; include `hitting`, `pitching`, and `scoring` basics.

## Acceptance

- File exists, is an ES5-safe IIFE, loads in Node, registers ≥48 questions.
- Every id is unique and follows the `q<CC><NN>` pattern.
- Every `chapter` value is one of ch05, ch06, ch07, ch08; every `tier` is `'sandlot'`.
